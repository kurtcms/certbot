...
if command -v docker-compose &> /dev/null; then
    docker-compose -f $dirpath/docker-compose.yml up -d
else
...
docker-compose -f $dirpath/docker-compose.yml restart nginx
...
docker-compose -f $dirpath/docker-compose.yml run --rm --entrypoint "certbot certonly --webroot \
